...
import { StudentResponse, StudentStandardResponse, StudentValueRequest } from '@/types/types'
...
export default function Student() {
  ...
  function filterSt(){
    setfilteredStandards(standards?.filter((item) => item.Level_number === selectedLevel))
  }
  async function updateResults(updatedStandards: StudentStandardResponse[]){
    try{
      const req: StudentValueRequest[] = updatedStandards
      .map(standard => ({
        student_id: +id,
        standard_id: standard.Standard.Id,
        value: standard.Standard.Has_numeric_value ? standard.Value : standard.Grade
      }))
  ...
  const handleStandardsChange = (updatedStandards: StudentStandardResponse[]) => {
  ...
  useEffect(() => {
    if (standards) {
      const uniqueLevels = Array.from(new Set(standards.map(item => item.Level_number)));
  ...
  return (
    <View className="bg-background-1 flex-1">
      <Stack.Screen
        options={{
          title: studentInfoLoading ? 'Загрузка информации об ученике...' : `${studentInfo?.last_name} ${studentInfo?.first_name} ${studentInfo?.patronymic}`,
          headerStyle: {
      ...
      <View className="w-full flex-row justify-between p-4">
        <CustomButton
          ...
          color='blue'
          size='sm'
          buttonText={selectedLevel !== -1 ? `${selectedLevel} класс` : 'Выбрать класс'}
