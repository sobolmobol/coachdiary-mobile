...
export default function CreateOrUpdateStudentScreen() {
  ...
  const isDisabledUpdate = () => {
    return studentInfo?.birthday === dateOfBirth.trim() && 
  ...
  const isDisabledCreate = () => {
    return dateOfBirth.trim().length === 0 || firstName.trim().length === 0 ||
    lastName.trim().length === 0 ||  
    !studentClass || classLetter.trim().length === 0 || 
    !gender
  }
        ...
        <View className="w-[90%] h-[90%] bg-background-1 shadow-md rounded-custom-big flex justify-around">
          <View className="w-full flex justify-between items-center gap-2">
            <View className='w-[90%] flex gap-3'>
              <Text className='text-primary-0 text-center text-m font-bold'>{id !== 'create' ? `${studentInfo?.last_name} ${studentInfo?.first_name.slice(0, 1)}. ${studentInfo?.patronymic.slice(0, 1)}.` : ''}</Text>
              ...
              <View className='w-[90%] flex p-2 gap-2 border-tertiary-0/50 bg-tertiary-0/30 rounded-custom'>
                ...
                <RadioGroup value={studentClass?.toString() ?? ''} onChange={(key) => {
                  setStudentClass(+key)
                  console.log(key)
                  }}>
                  <View className="flex-row justify-start flex-wrap gap-2 p-2">
                    {[ ...Array(11).keys() ].map( i => i+1).map((item) => {
                      return(
              ...
              <View className='w-[90%]'>
                <CustomButton
                  ...
                  isDisabled={id === 'create' ? isDisabledCreate() : isDisabledUpdate()}
